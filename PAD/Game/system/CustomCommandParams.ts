...
class CustomCommandParams_15001 {
    beiJingTu: string; // = ""; 背景图片
    fangKuaiTuPian: string; // = ""; 方块图片
    gameOvarTu: string; // = ""; 游戏结束图片
    zanTingTu: string; // = ""; 暂停图片
    jianGeChuShi: number; // = 300; 初始下降间隔
    fenJianGe: number; // = 0; 每分减少间隔
    fenShuBianLiang: number; // = 0; 分数变量
}
/**
* 自定义指令 15002-开始战斗
*/
class CustomCommandParams_15002 {
    enemyParty: number; // = 0; 敌人队伍
    time: number; // = 10000; 转珠时长（毫秒）
    row: number; // = 5; 元素行数
    col: number; // = 6; 元素列数
    elementWidth: number; // = 100; 元素宽
    elementHeight: number; // = 100; 元素高
    UseElement: string; // = "1-2-3-4-5-6"; 使用的元素（格式：元素编号-元素编号）
}
/**
* 自定义指令 15003-结束战斗
*/
