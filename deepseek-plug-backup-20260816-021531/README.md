...
文档实测「原地重写 touch」在**非预览**状态下 60s 内**不会**触发重编译，并列出两种可能：

1. 编辑器只在**游戏预览运行中**才激活文件监听；
2. 编辑器根本不监听外部写入，Trae 靠别的机制感知。

**因此：安装插件前，必须先在「预览运行中」验证原地写能否触发重编译。** 本包附带验证脚本：

```powershell
# 1) 先在 GameCreator 里运行游戏预览
# 2) 再执行（默认测 PADPuzzle.ts → PADPuzzle.js，观察 60s）
node C:\PAD\deepseek-plug\scripts\verify-recompile.mjs
```

结果解读：

- 产物 mtime/size 变化 → 走 C2 即可（本插件默认行为），安装即可用。
- 预览运行中仍未变化 → 编辑器不监听外部 `LAST_WRITE`，需要改走「主动通知编辑器重编译」（成本更高，需摸清 GameCreator 编辑器接口）。

脚本只原地重写相同字节、不改内容，可安全重跑。
...
- id: write-touch
  config:
    ...
    touchMode: rewrite     # rewrite = 原地重写（默认，可靠）；mtime = 仅 utimes（省 I/O，Windows 上可能不触发 LAST_WRITE）
```
...
| `touchMode` | `"rewrite"` | `rewrite`：字节级原地重写；`mtime`：仅更新 mtime（备选，大文件场景） |
...
├─ scripts/verify-recompile.mjs# §5.4 关键假设验证脚本
├─ README.md
└─ LICENSE
```
