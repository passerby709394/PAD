 ...
 * - touchMode:  "rewrite" = 字节级原地重写（C2，可靠触发 LAST_WRITE，默认）；
 *               "mtime"   = 仅更新 mtime（`utimes`，I/O 更小，但 Windows 上
 *                           不一定触发 LAST_WRITE，作为大文件/备选）。
 */
export function apply(ctx, config) {
  const resolved = {
    enabled: config?.enabled ?? true,
    extensions: normalizeExtensions(config?.extensions ?? [".ts"]),
    touchMode: config?.touchMode === "mtime" ? "mtime" : "rewrite"
  };
...
function wrapFs(ctx, resolved) {
  ...
  async function touchAndFixVersion(target, outcome) {
    ...
    try {
      ...
      if (!path) return;
      await emitLastWrite(path, resolved.touchMode);
...
/** 按 touchMode 补发 LAST_WRITE。 */
async function emitLastWrite(path, touchMode) {
  if (touchMode === "mtime") {
    const now = new Date();
