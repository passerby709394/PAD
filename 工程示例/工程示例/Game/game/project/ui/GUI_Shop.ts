...
class GUI_Shop extends GUI_11 {
    ...
    constructor() {
        ...
        // 记录颜色
        this.itemInitColor = this.itemName.color;
        // 开启监听
        this.listenQueue();
    }
    /**
    ...
    public initGoodsList() {
        ...
        for (let i = 0; i < dataArr.length; i++) {
            ...
            let moduleID = goods.isEquip ? 9 : 1;
            let gooldsID = goods.isEquip ? goods.equip : goods.item;
            ...
            let playerItemDS = ProjectPlayer.getItemDS(item.id, goods.isEquip);
    ...
    private onGoodsCreate(ui: GUI_1003, data: ListItem_1003, index: number): void {
        let itemDS = data.data as DataStructure_shopItem;
        if (itemDS && itemDS.isEquip) {
            ui.itemName.color = GUI_Manager.getEquipNameColor(itemDS.equip);
        }
    }
    private onSellItemCreate(ui: GUI_1003, data: ListItem_1003, index: number): void {
        let itemDS = data.data as DataStructure_packageItem;
        if (itemDS && itemDS.isEquip) {
    ...
    private buyGoods(selectedItem: ListItem_1003, buyNum: number) {
        ...
        let itemID = itemDS.isEquip ? itemDS.equip : itemDS.item;
        // 播放 确认音效
        GameAudio.playSE(ClientWorld.data.sureSE);
        // 扣除对应货币
    ...
    private reduceGoodsNum(buyNum: number) {
        ...
        // 刷新拥有
        let itemID = data.isEquip ? data.equip : data.item;
        let playerItemDS = ProjectPlayer.getItemDS(itemID, data.isEquip);
    ...
    private sellItem(selectedItem: ListItem_1003, sellNum: number) {
        ...
        let itemPrice = Math.floor(Number(selectedItem.itemPrice));
        let itemID = itemDS.isEquip ? itemDS.equip.id : itemDS.item.id;
        // 播放 确认音效
        GameAudio.playSE(ClientWorld.data.sureSE);
        // 增加对应货币
    ...
    private refreshGoodsListView() {
        for (let i = 0; i < this.goodsList.length; i++) {
            // 获取对应商品的界面
            let goodsUI: GUI_1010 = this.goodsList.getItemUI(i) as any;
            // 获取对应商品的数据
    ...
    private refreshSellItemListView() {
        for (let i = 0; i < this.sellItemList.length; i++) {
            // 获取对应商品的界面
            let sellItemUI: GUI_1010 = this.sellItemList.getItemUI(i) as any;
            // 获取对应商品的数据
    ...
    private refreshItemInfo(list: UIList = this.goodsList) {
        ...
        // 已选中道具的情况：显示该道具详情
            ...
            let isEquip: boolean;
            let item: Module_Item | Module_Equip;
            if (this.typeTab.selectedIndex == 0) {
                let itemDS: DataStructure_shopItem = selectedItem.data;
            ...
            if (isEquip) {
                ...
                this.itemName.color = GUI_Manager.getEquipNameColorByInstance(item as Module_Equip);
            }
            else {
